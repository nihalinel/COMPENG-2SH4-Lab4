...
Matrix::~Matrix()
{
    // Check if matrixData is not nullptr
    if (matrixData != nullptr)
    {
        ...
        // Set matrixData to nullptr to avoid double deletion
        matrixData = nullptr;
    }
}
...
int Matrix::getElement(int i, int j)
{
	...
	if (i >= rowsNum || j >= colsNum || i < 0 || j < 0)
	{
		throw std::out_of_range("Invalid indexes.");
	}
	else
...
void Matrix::addTo( Matrix &m ){
	...
	// IMPLEMENT YOUR CODE BELOW
	//cout << m.getsizeofrows() << " " << m.getsizeofcols() <<endl;
	//cout <<rowsNum << " " << colsNum << " " <<endl;

	if (m.getsizeofcols() != colsNum || m.getsizeofrows() != rowsNum)
	{
...
Matrix Matrix::subMatrix(int i, int j){
    ...
    for (rows = 0; rows <= i; rows++)
	{
		...
		{
            submatrix.matrixData[rows][cols] = matrixData[rows][cols];
        }
    }
